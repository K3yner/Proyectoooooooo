...
import datetime
from tkcalendar import Calendar #Este módulo no viene con python. Hay que descargarlo
import moduloProductos as pr
import moduloContabilidad as cnt
import moduloGeneral as gen

#VENTANAS TEMPORALES
#Esta ventana debe reemplazarse por la ventana de "Productos"
#de la estructura principal en todos los widgets de productos
pagina12 = tk.Tk() 
pagina12.title("Productos")
pagina12.grid()
pagina12.geometry("500x500")
#Esta ventana debe reemplazarse por la ventana de "Contabilidad"
#de la estructura principal en todos los widgets de productos
pagina1 = tk.Tk()
...
#PRODUCTOS
#Diccionario de productos vacíos
...
buscador_productos.insert(0,"🔎                       Buscar un producto") #Texto de relleno
buscador_productos.bind("<FocusIn>", lambda x:gen.borrar_texto(x,buscador_productos)) #Borrar el texto de relleno si se hace clic
buscador_productos.bind("<FocusOut>", lambda x:pr.texto_buscador_productos(x,buscador_productos)) #Regresar el texto de relleno al salir del buscador
buscador_productos.bind("<Return>", lambda x:pr.buscar_producto(x,buscador_productos,productos,pagina12)) #Activar la función buscar al presionar enter
...
boton_Fecha = tk.Button(pagina1, text = "Fecha:", command=lambda:cnt.mostrar_calendario(fecha_Label))
...
Añadir_Ingreso_Diario = tk. Button(pagina1, text="Añadir Ingreso", command= lambda: cnt.añadirIngreso(ventas,fecha,productos,popUp_ingresos))
Añadir_Ingreso_Diario.grid(row=2, column=1)

#Botón Añadir pago
Añadir_Pago_Diario = tk. Button(pagina1, text="Añadir Pago", command= lambda: cnt.añadirPago(popUp_pagos,recurrentes,pagos,marcar_Recurrente,marcar_Inversion,fecha,inversiones))
