...
def mostrar_calendario(fecha_Label,calendario,cal):
    #Mostrar popUp
    calendario.deiconify()
    #Colocar calendario
    cal.pack()
    #Botón seleccionar fecha
    seleccionar = tk.Button(calendario, text="Seleccionar Fecha", command=lambda:seleccionar_fecha(calendario,cal,seleccionar,fecha_Label))
    seleccionar.pack()
...
def añadirIngreso(ventas,cal,productos,popUp_ingresos):
    ...
    #Botones aceptar y cancelar
        aceptar = tk.Button(popUp_ingresos, text = "Aceptar", command = lambda: aceptarIngreso(popUp_ingresos,cajaTexto2,producto,productos,ventas,nombres_productos,cal))
...
def aceptarIngreso(popUp_ingresos,cajaTexto2,producto,productos,ventas,nombres_productos,cal):
    ...
    try:
        ...
        #Agregar la venta al diccionario de ventas
            ...
            ventas.append([producto, cantidad, ingreso,cal.selection_get()])
...
def añadirPago(popUp_pagos,recurrentes,pagos,marcar_Recurrente,marcar_Inversion,cal,inversiones):
    ...
    #Botones aceptar y cancelar
    aceptar = tk.Button(popUp_pagos, text = "Aceptar", command = lambda: aceptarPago(popUp_pagos,cajaTexto1,cajaTexto2,pagos,marcar_Recurrente,marcar_Inversion,cal,recurrentes,inversiones))
...
def aceptarPago(popUp_pagos,cajaTexto1,cajaTexto2,pagos,marcar_Recurrente,marcar_Inversion,cal,recurrentes,inversiones):
    try:
        #Nombre del pago
        pago = cajaTexto1.get()
        ...
        #Agregar el pago al diccionario de pagos, con su monto
            ...
            pagos[pago] = [monto,cal.selection_get()]
    ...
    if marcar_Inversion.cget("text") == "✓":
        inversiones[pago] = [monto,cal.selection_get()]
    print(inversiones)
...
def añadirPago_mensual(popUp_pagos,recurrentes,pagos,marcar_Recurrente,marcar_Inversion,cal,inversiones,calendario):
    añadirPago(popUp_pagos,recurrentes,pagos,marcar_Recurrente,marcar_Inversion,cal,inversiones)
    fecha_pago = tk.Button(popUp_pagos,text = "Fecha",command=lambda:mostrar_calendario(fecha_Label,calendario,cal))
...
def añadirIngreso_mensual(ventas,cal,productos,popUp_ingresos):
    añadirIngreso(ventas,cal,productos,popUp_ingresos,)
    fecha_pago = tk.Button(popUp_ingresos,text = "Fecha",command=lambda:mostrar_calendario(fecha_Label,calendario,cal))
    fecha_pago.grid(row = 4, column=1)
    string_fecha = gen.fecha_letras(datetime.date.today())
